...
var LONG_INFINITY = bignum("9223372036854775807");
var LONG_NEG_INFINITY = bignum("9223372036854775809");
...
function deserialize(b, nanos2date, flipTables, emptyChar2null, long2bignum) {
	...
	function rInt64() { // bignum or closest number to 64 bit int
		if (long2bignum === true) {
			...
			if (val.eq(LONG_INFINITY)) {
				return Infinity;
			}
			if (val.eq(LONG_NEG_INFINITY)) {
				return -Infinity;
			}
			return val;
		} else {
...
function isListOfSamePrimitiveType(elements, valuetype) {
	if (valuetype === "list" || valuetype === "typedlist" || valuetype === "string" || valuetype === "dict") {
		return false;
	}
		...
		case 'typedlist':
			{
				...
				if (vt === "symbol") {
					for (var i = 0; i < x.length; i++) {
						var encx = unescape(encodeURIComponent(x[i]));
						n += 1 + encx.length;
					}
				} else {
