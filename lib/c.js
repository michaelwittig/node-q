...
function isListOfSamePrimitiveType(elements, valuetype) {
	if (valuetype === "list" || valuetype === "typedlist" || valuetype === "mixedlist" || valuetype === "string" || valuetype === "dict") {
		return false;
	}
...
function calcN(x, dt) {
	...
	switch (t) {
		...
		case "string": // JavaScript only type => list of char
			{
				var n=Buffer.byteLength(x, 'utf8') + (x[0] === "`" ? 1 : 6);
				return n;
			}
